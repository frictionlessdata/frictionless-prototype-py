...
def test_table_headers_unicode():
    with Table('data/table_unicode_headers.csv', headers=1) as table:
...
def test_table_headers_stream_context_manager():
    ...
    with Table(source, headers=1, format='csv') as table:
        assert table.headers == ['id', 'name']
...
def test_table_headers_json_keyed():
    ...
    with Table(source, headers=1, format='json') as table:
        assert table.headers == ['id', 'name']
        assert list(table.iter(keyed=True)) == [
            {'id': 1, 'name': 'english'},
            {'id': 2, 'name': '中国人'},
